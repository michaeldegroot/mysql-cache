...
exports.flush = cb => {
    ...
    if (typeof cb !== 'function') {
        util.error('flush was called without a callback which is unsupported, use a callback for your flush function')
    } else {
        cacheProvider.run('flush', null, null, null, (err, result) => {
            if (err) {
                util.error(err, cb)
            } else {
...
exports.delKey = (id, params, cb) => {
    ...
    if (typeof cb !== 'function' || cb === undefined) {
        util.error('delKey was called without a callback which is unsupported, use a callback for your delKey function')
    } else {
        if (typeof id === 'object') {
            params = id['params']
            id     = id['sql']
    ...
    exports.pool.getConnection((err, connection) => {
        exports.pool.end(err => {
            if (err) {
                ...
                util.trace(`${poolPrefix}: Pool is killed`)
